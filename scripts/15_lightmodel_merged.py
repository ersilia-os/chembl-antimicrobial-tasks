...
# Assays with a PubChem counterpart are modelled independently — exclude from merging
pubchem_assays = load_pubchem_assays(pathogen_code)
pubchem_excluded_rows = not_accepted[not_accepted["assay_id"].isin(pubchem_assays)].copy() if pubchem_assays else pd.DataFrame()
if pubchem_assays:
    not_accepted = not_accepted[~not_accepted["assay_id"].isin(pubchem_assays)].copy()
...
for _, row in pubchem_excluded_rows.iterrows():
    merging_analysis.append({
        ...
        "group_size": 0,
        "group_compounds": 0,
        "failure_reason": "excluded_pubchem_assay",
        "group_keys": "",
    })
