...
output_dir = os.path.join(CONFIGPATH, "statistical_processed")

def annotate_activities_with_activity_flag():
    print("Loading standard text table...")
    df = pd.read_csv(os.path.join(CONFIGPATH, "llm_processed","standard_text.csv"))
    std_text = {}
    for v in df.values:
        if v[1] == 0:
            continue
    ...
    df = pd.read_csv(os.path.join(CONFIGPATH,"llm_processed", "activity_comments.csv"), low_memory=False)
    act_comm = {}
    ...
    df = pd.read_csv(os.path.join(CONFIGPATH,"chembl_activities", "activities.csv"), low_memory=False)
    print(df.columns)
    flags = []
    ...
    df.to_csv(os.path.join(output_dir, "activities.csv"), index=False)
...
def standard_units_resolver_with_statistical_test(directions, rewrite=False):
    df = pd.read_csv(os.path.join(CONFIGPATH,"llm_processed", "activity_std_units.csv"), dtype=str)
    columns = list(df.columns)
    if "activity_direction" not in columns:
        df["activity_direction"] = ""
        df.to_csv(os.path.join(output_dir, "activity_std_units.csv"), index=False)
    if rewrite:
        df["activity_direction"] = ""
        df.to_csv(os.path.join(output_dir,"activity_std_units.csv"), index=False)
...
if __name__ == "__main__":
    ...
    standard_units_resolver_with_statistical_test(directions, rewrite=False)
