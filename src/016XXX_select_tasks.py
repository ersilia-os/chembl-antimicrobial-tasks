...
modelable_tasks = set(dm[(dm["auroc_avg"] >= 0.7) & (dm["ratio"] <= 0.5)]["task"])  # If AUROC < 0.7 or ratio > 0.5, the task is not modelable - then we check for distinguishability
# distinguishable_tasks = ...

print(modelable_tasks)
